  ...
  <div style="display: flex" class="header">
    <router-link to="/">
      <div class="title-wrapper">
        <img src="@/assets/logo/logoImg.png" class="logo"/><span class="title">双碳采购网</span>
      </div>
    </router-link>
    <el-tabs class="tabs" style="width: 30%;" :stretch="true" @tab-click="handleTabChange">
      ...
      <el-tab-pane label="用户社区" name="/community/list"/>
    </el-tabs>


    <div class="login">
      <div style="padding-right: 30px;">
        <BidMessageList></BidMessageList>
      </div>
      ...
      <div class="user" style="width: 100px">
        <span v-if="realName === ''" @click="handleClickUserInfo">请登录</span>
        <el-dropdown v-else>
          <span @click="handleClickUserInfo">{{ realName}}</span>
          <template #dropdown>
            <el-dropdown-menu>
...
import BidMessageList from '@/views/bidMessage/BidMessageList.vue'

export default {
  name: 'Header',
  components: { BidMessageList },
  data() {
    return {
      routers: ['/index', '/policylaw/list', '/complaint/list', '/member/list', '/community/list']
    }
  },
  methods: {
    async logout() {
