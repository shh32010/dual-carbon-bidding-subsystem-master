...
export default {
  ...
  methods: {
    ...
    getStatusValue(key) {
      const values = this.status.filter(x => x.key === key);
  ...
  mounted() {
    const that = this;
    getComplaint(that.id).then(response => {
      that.$nextTick(() => {
        that.detailData = response.data
        that.bidTitle = that.detailData.title;
        that.bidPublishDatetime = that.parseTime(that.detailData.createTime, '{y}年{m}月{d}日');
        that.bidContent = that.detailData.content;
        that.bidFiles = that.detailData.filesList;
        that.statusVal = that.detailData.status;
